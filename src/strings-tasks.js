...
function formatTime(minutes, seconds) {
  const min = minutes > 10 ? minutes : String(minutes).padStart(2, '0');
  const sec = seconds > 10 ? seconds : String(seconds).padStart(2, '0');
  return `${min}:${sec}`;
}
...
function encodeToRot13(str) {
  ...
  const ciphers =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklmABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let result = '';
  let index = 0;
  for (let i = 0; i < str.length; i += 1) {
    index = characters.indexOf(str[i]);
    result += index >= 0 ? ciphers[index] : str[i];
  }
  return result;
}
